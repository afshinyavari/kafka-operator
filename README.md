...
- **Prometheus metrics across the data plane** — `spec.metricsConfig` opts in `jmx_prometheus_javaagent` + a `ServiceMonitor` on every Kafka broker pool, the Kroxylicious proxy (native `/metrics` on 9190), Cruise Control / MirrorMaker2 / KafkaConnect (JMX on 9101). `ServiceMonitor` is applied via `OptionalResourceApplier` and silently no-ops on clusters without the Prometheus Operator CRD.
