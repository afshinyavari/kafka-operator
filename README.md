...
- **Prometheus metrics across the data plane** — `spec.metricsConfig` opts in `jmx_prometheus_javaagent` + a `ServiceMonitor` on every Kafka broker pool, the Kroxylicious proxy (native `/metrics` on 9190), Cruise Control / MirrorMaker2 / KafkaConnect (JMX on 9101), and Apicurio registry (Quarkus `/metrics` on 8080). `ServiceMonitor` is applied via `OptionalResourceApplier` and silently no-ops on clusters without the Prometheus Operator CRD.
- **Pod-level resilience baked in** — every operator-built workload ships with liveness + readiness probes, `maxUnavailable=1` `PodDisruptionBudget`s on multi-replica deployments (proxy, Connect, MM2, Apicurio) in addition to the per-pool broker PDB, and the operator-default pod security context (`runAsNonRoot`, `seccomp: RuntimeDefault`, drop ALL caps, no privilege escalation). Every Dockerfile we build uses a Red Hat UBI base — including Kroxylicious, compiled from source onto `ubi9/openjdk-21`.
...
- **MirrorMaker2 with Apicurio schema mirroring** — `MirrorMaker2` CRD deploys MM2 in dedicated mode. Each end (source/target) is independently a managed `KafkaCluster` ref or an external endpoint. When both ends carry Apicurio, an opt-in custom Connect SMT (`ApicurioSchemaTransferSmt`) rewrites the envelope's globalId per record so consumers can decode mirrored payloads; it authenticates to a managed cluster's RBAC-proxied registry via OAuth2 client-credentials (`schemaRegistryAuthSecretRef`). The SMT is standalone-friendly (e.g. Strimzi `KafkaMirrorMaker2`): `source.format`/`target.format` select Apicurio or Confluent per side (Confluent source → Apicurio target included) and `<side>.ssl.*` keys enable mTLS to the registries. `make -C kind mm2-mirror-test` is a full data + schema mirror e2e. See [api-reference.md#mirrormaker2](docs/api-reference.md#mirrormaker2).
- **Kafka Connect (distributed) + declarative connectors** — `KafkaConnect` CRD provisions a distributed Connect worker cluster (Deployment + REST Service on 8083 + 3 internal-topic CRs + ConfigMap) attached to a managed or external Kafka. Plugins arrive via PVC, custom image, ConfigMap, or Secret — composable. `KafkaConnector` CRs describe individual connectors; the reconciler PUTs configs to the Connect REST API, reverts external drift back to spec, and surfaces per-task status (RUNNING/PAUSED/STOPPED/FAILED). `configFrom.secretRef` keeps credentials out of the CR. `make -C kind kafka-connect-smoke-test` exercises the full lifecycle. See [api-reference.md#kafkaconnect](docs/api-reference.md#kafkaconnect).
...
- **Unified audit logging** — every authorisation decision made by Kroxylicious and the Apicurio rbac-proxy is captured as one JSON line per request on the dedicated SLF4J channel `kafka-audit` (always on). Opt in to a Kafka-topic sink via `KafkaCluster.spec.audit.kafkaTopic` and the operator upserts a `__audit` topic and ships records over the proxy's mTLS client cert, direct to the broker `INTERNAL` listener (no Kroxylicious loop). See [docs/audit.md](docs/audit.md).
...
│       ├── ApicurioSchemaTransferSmt.java  # Envelope rewrite + reference DFS + LRU/negative cache
│       ├── EnvelopeCodec.java              # APICURIO (int64) / CONFLUENT (int32) wire formats
